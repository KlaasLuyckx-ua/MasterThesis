...
n_points = 100  # number of points in parameter scan
...
M_range = 10 .^ range(log10(1e6), log10(1e8), length=n_points)
...
for J_loc in J_range
    ΔV_max, _, _, _, _, _, _ = calculate_critical_ΔV_JJ(J_loc, γ, B_21, M, T, Δ, κ, n_avg, δ)
    push!(all_data, DataFrame(ScanType="J", X_Value=J_loc, dV_Max=ΔV_max))
end

println("Scanning γ...")
for γ_loc in γ_range
    ΔV_max, _, _, _, _, _, _ = calculate_critical_ΔV_JJ(J, γ_loc, B_21, M, T, Δ, κ, n_avg, δ)
    push!(all_data, DataFrame(ScanType="γ", X_Value=γ_loc, dV_Max=ΔV_max))
end

println("Scanning n_avg...")
for n_avg_loc in n_avg_range
    ΔV_max, _, _, _, _, _, _ = calculate_critical_ΔV_JJ(J, γ, B_21, M, T, Δ, κ, n_avg_loc, δ)
...
for M_loc in M_range
    ...
    ΔV_max, _, _, _, _, _, _ = calculate_critical_ΔV_JJ(J, γ, B_21, M_loc, T, Δ, κ_loc, n_avg, δ)
...
for B_21_loc in B_21_range
    ...
    ΔV_max, _, _, _, _, _, _ = calculate_critical_ΔV_JJ(J, γ, B_21_loc, M, T, Δ, κ_loc, n_avg, δ)
...
for Δ_loc in Δ_range
    ...
    ΔV_max, _, _, _, _, _, _ = calculate_critical_ΔV_JJ(J, γ, B_21, M, T, Δ_loc, κ_loc, n_avg, δ)
...
for T_loc in T_range
    ...
    ΔV_max, _, _, _, _, _, _ = calculate_critical_ΔV_JJ(J, γ, B_21, M, T_loc, Δ, κ_loc, n_avg, δ)
...
output_file = "Param_scan/Data/Master_Stability_Results_2_Site_s.csv"
...
Δ_range = range(-100, -40, length=n_points)
T_range = range(10, 40, length=n_points)
...
output_file = "Param_scan/Data/Master_Stability_Results_2_Site_l.csv"
CSV.write(output_file, master_df)
